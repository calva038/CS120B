/*
[calva038]_lab[1]_part[3].c-[8/10/18]
*Name & E-mail: Christian Alvarado, calva038@ucr.edu
*CS Login: calva038
*Partner(s) Name & E-mail: N/A
*Lab Section: B22
*Assignment: Lab #1  Exercise #3
*Exercise Description: Sets PC0-PC3 to display the number of cars based on PA0-PA3 inputs, and sets PC7 to 1 if all spots are filled
*
...
int main(void)
{
	...
	while(1)
	{
		...
		if (PINA == 0x00){
			cntavail = (cntavail & 0x00); // Sets tmpC to ccccccc0 (clear rightmost bit)
		}
		else if ((PINA == 0x01) || (PINA == 0x02) || (PINA == 0x04) || (PINA == 0x08)){
			cntavail = (cntavail & 0x00) | 0x01; // Sets tmpC to 00000001 (sets PC0 to 1)
		}
		else if ((PINA == 0x03) || (PINA == 0x05) || (PINA == 0x06) || (PINA == 0x09) || (PINA == 0x0A) || (PINA == 0x0C)){
			cntavail = (cntavail & 0x00) | 0x02; // Sets tmpC to 00000010 (sets PC1 to 1)
		}
		else if ((PINA == 0x07) || (PINA == 0x0B) || (PINA == 0x0D) || (PINA == 0x0E)){
			cntavail = (cntavail & 0x00) | 0x03; // Sets tmpC to 00000011 (sets PC0 and PC1 to 1)
		}
		else if (PINA == 0x0F){
			cntavail = (cntavail & 0x00) | 0x84; // Sets tmpC to 10000100 (sets PC2 to 1)
		}
		PORTC = cntavail;
	}
	return 0;
